...
// WhackAMole 
// 
// See https://chrislaffra.com/whackamole
//
// From here on, all documentation below was generated by SourceGraph's Cody AI.
//
// This Chrome Extension handles hiding ads on web pages.
//
...
(function WhackAMole() {
    ...
    */
    function hide(mole, explanation) {
        if (mole.attr('whacked')) return;
        mole.attr('whacked', true);

        if (mole.css("position") === "absolute") {
            const height = mole.css("height");
            mole.css("height", 26);
            mole.children().css("opacity", 0);
            mole.append($("<span>")
                .css({
                    position: "absolute",
                ...
                .text("ad")
                .click(() => mole.css("height", height).children().css("opacity", 1))
            );
        }
        else {
            var url = mole.attr('src') || mole.attr('data') || mole.attr('href') || '';
            if ((!url || !sameDomain(url))) {
                log(`hide ${mole.attr("whacked")} ${explanation}`);
                var wrapper = createWrapper(mole, function() {
                    log(`Show Ad - ${explanation}`)
    ...
    */
    function hide_matching(parentSelector, child, explanation) {
        child.each(function() {
            const mole = $(this).closest(parentSelector);
            hide(mole, `matching ${parentSelector} for ${child.prop("tagName")} - ${explanation}`);
        })
    }
    ...
    */
    function hide_selector(selector) {
        $(selector).each(function() { hide($(this), `hide_selector ${selector}`); });
    }
    ...
    */
    function run() {
        ...
        if (domain.match("twitter.com")) {
            hide_matching('div[data-testid="cellInnerDiv"]', $('span').filter(function(){
                return $(this).text() === 'Promoted';
            }), "Twitter Ad");
        }
